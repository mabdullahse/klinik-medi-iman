import type { Metadata } from "next";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { getDictionary } from "@/lib/i18n";
import { getLocalePath } from "@/lib/navigation";
import type { Locale } from "@/lib/i18n";

export const dynamicParams = false;

export async function generateStaticParams() {
  return [{ locale: "ms" }, { locale: "en" }];
}

export async function generateMetadata({
  params,
}: {
  params: Promise<{ locale: Locale }>;
}): Promise<Metadata> {
  const { locale } = await params;
  const dict = await getDictionary(locale);
  return {
    title: dict.meta.generalHealth.title,
    description: dict.meta.generalHealth.description,
  };
}

export default async function GeneralHealthPage({
  params,
}: {
  params: Promise<{ locale: Locale }>;
}) {
  const { locale } = await params;
  const dict = await getDictionary(locale);
  const d = dict.generalHealthPage;

  return (
    <div className="bg-cream min-h-screen">
      {/* Hero */}
      <section
        className="relative min-h-[50vh] flex items-center   overflow-hidden"
        style={{
          background:
            "linear-gradient(rgba(123,28,46,0.82),rgba(123,28,46,0.82)), url('/assets/general-health.png') center/cover no-repeat",
          backgroundColor: "#7B1C2E",
        }}
      >
        <div className="relative container-custom text-center">
          <span className="inline-block text-gold font-semibold text-xs uppercase tracking-widest mb-4">
            {d.hero.label}
          </span>
          <div className="gold-divider mx-auto mb-5" />
          <h1 className="heading-xl text-white font-playfair mb-5">
            {d.hero.heading}
          </h1>
          <p className="text-white/80 max-w-2xl mx-auto leading-relaxed mb-8">
            {d.hero.description}
          </p>
          <Link href={getLocalePath("/temujanji", locale)}>
            <Button variant="gold" size="lg">
              {d.hero.cta}
            </Button>
          </Link>
        </div>
      </section>

      {/* Services Section */}
      <section className="section-padding bg-white">
        <div className="container-custom">
          <div className="text-center mb-12">
            <span className="inline-block text-gold font-semibold text-xs uppercase tracking-widest mb-3">
              {d.servicesHeading}
            </span>
            <div className="gold-divider mx-auto" />
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {d.services.map((category) => (
              <div
                key={category.cat}
                className="bg-white rounded-2xl shadow-sm border border-gold/10 p-6 hover:shadow-md hover:border-gold/30 transition-all"
              >
                <h3 className="font-playfair font-bold text-maroon text-lg mb-4">
                  {category.cat}
                </h3>
                <ul className="space-y-2">
                  {category.items.map((item) => (
                    <li key={item} className="flex items-start gap-2 text-sm text-gray-600">
                      <span className="text-gold mt-0.5 shrink-0">✦</span>
                      {item}
                    </li>
                  ))}
                </ul>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="section-padding bg-maroon text-white">
        <div className="container-custom text-center">
          <h2 className="heading-lg font-playfair text-white mb-4">
            {d.cta.heading}
          </h2>
          <p className="text-white/80 max-w-xl mx-auto mb-8">
            {d.cta.description}
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href="https://wa.me/601169863974"
              target="_blank"
              rel="noopener noreferrer"
            >
              <Button variant="whatsapp" size="lg">
                {d.cta.btnWa}
              </Button>
            </a>
            <Link href={getLocalePath("/temujanji", locale)}>
              <Button variant="gold" size="lg">
                {d.cta.btnAppt}
              </Button>
            </Link>
          </div>
        </div>
      </section>
    </div>
  );
}
